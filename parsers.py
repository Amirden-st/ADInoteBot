...
class MessageParser:
    ...
    def get_note_dict(self):
        note_parts = re.search(r'([^(?:;;)]+);?;?([^(?:;;)]+)?;?;?([^(?:;;)]+)?', self._message.text).groups()
        note_parts = list(filter(lambda el: el, note_parts))
        note_dict = {'title':None, 'content':None, 'category_name':'non_category'}
        if len(note_parts) == 1:
        ...
        return note_dict
note_parts = re.search(r'([^(?:;;)]+);?;?([^(?:;;)]+)?;?;?([^(?:;;)]+)?', 'dawwaeda;;aedda;;efadeafaef;;dsas').groups()
